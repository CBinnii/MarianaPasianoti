...
abstract class Odin_Front_End_Form {
	...
	protected $custom_errors = array(
		...
		'valid_email' => "must be an email address valid."
	);
	 ...
	 * Set form redirect.
	 *
	 * @param String $redirect Submit form.
	 *
	 * @return void
	 */
	...
	protected function process_buttons() {
		...
		if ( ! empty( $this->buttons ) ) {
			...
			$html .= '<button type="submit" class="btn btn-primary">' . __( 'Enviar', 'odin' ) . '</button>';
		}
	...
	protected function display_success_message() {
		...
		if ( isset( $_GET['success'] ) && 1 == $_GET['success'] ) {
			...
			if ( ! empty( $this->success ) ) {
				...
				$html .= '<p>' . __( 'Formulário enviado com sucesso!', 'odin' ) . '</p>';
			}
			$html .= '</div>';
		}
		...
		if ( ! empty( $this->fields ) && ! empty( $data ) ) {
			foreach ( $this->fields as $fieldset ) {
				foreach ( $fieldset['fields'] as $field ) {
					...
					if ( $required && empty( $data[ $id ]) && $type != 'email' ) {
						$errors[] = sprintf( __( '%s ' . $this->custom_errors['required'], 'odin' ), '<strong>' . $label . '</strong>' );
					}

					switch ( $type ) {
						case 'email':
							if ( ! is_email( $value ) )
								$errors[] = sprintf( __( '%s ' . $this->custom_errors['valid_email'], 'odin' ), '<strong>' . $label . '</strong>' );
							break;
